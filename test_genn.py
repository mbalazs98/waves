...
from pygenn import (GeNNModel, VarLocation, init_var,
                    init_postsynaptic, init_weight_update, create_sparse_connect_init_snippet, create_var_init_snippet,
...
EE_synapse_init = init_weight_update("StaticPulseDendriticDelay", {}, {"g": EXCITATORY_WEIGHT, "d": init_var(calc_dist, {"delay": EE_dist*conduction_delay, "grid_num_x": int(n_side_E)})})
EI_synapse_init = init_weight_update("StaticPulseDendriticDelay", {}, {"g": EXCITATORY_WEIGHT, "d": init_var(calc_dist_resize, {"delay": II_dist*conduction_delay, "grid_num_x": int(n_side_E), "grid_num_x2": int(n_side_I)})})
II_synapse_init = init_weight_update("StaticPulseDendriticDelay", {}, {"g": INHIBITORY_WEIGHT, "d": init_var(calc_dist, {"delay": II_dist*conduction_delay, "grid_num_x": int(n_side_I)})})
IE_synapse_init = init_weight_update("StaticPulseDendriticDelay", {}, {"g": INHIBITORY_WEIGHT, "d": init_var(calc_dist_resize, {"delay": EE_dist*conduction_delay, "grid_num_x": int(n_side_I), "grid_num_x2": int(n_side_E)})})
...
axes[2].plot(rate_bin_centres, inh_rate * (1000.0 / bin_size) * (1.0 / NUM_INHIBITORY))
